...
blk_t *iwalk(dinode_t *file, uint32_t blk_no) {
  ...
  blk_no -= NDIRECT;//减去直接寻址的块数，得到间接寻址的块数
  if (blk_no < NINDIRECT) {
    // indirect address
     uint32_t *indirect_block=NULL;//初始化间接寻址块的指针
     if(file->addrs[NDIRECT]==0){
      file->addrs[NDIRECT]=balloc();
     }
  indirect_block=bget(file->addrs[NDIRECT])->u32buf;
  if(indirect_block[blk_no]==0){
    indirect_block[blk_no]=balloc();
  }
  return bget(indirect_block[blk_no]);
  }
  panic("file too big");
}
...
void iappend(dinode_t *file, const void *buf, uint32_t size) {
  ...
  uint32_t block_num=file->size/BLK_SIZE;
  uint32_t offset=file->size%BLK_SIZE;
  uint32_t cur_size=file->size;
  while(size>0){
    //获取file对应逻辑块的地址
    blk_t* block=iwalk(file,block_num);

    //计算本次最多可写入字节数
    uint32_t  write_size=MIN(size,BLK_SIZE-offset);

    //进行数据复制
    memcpy(block->u8buf+offset,buf,write_size);
    cur_size+=write_size;
    offset+=write_size;
    
    //更新文件大小和偏移量
    size-=write_size;
     buf = ((char *)buf + write_size);// buf = (char *)buf + write_size;
