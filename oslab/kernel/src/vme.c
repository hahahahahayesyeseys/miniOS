...
void init_page() {
    //init kpd and kpt, identity mapping of [0 (or 4096), PHY_MEM)s
  extern char end;
  ...
  //  init free memory at [KER_MEM, PHY_MEM), a heap for kernel
...
void *kalloc() {
  //   alloc a page from kernel heap, abort when heap empty
...
PD *vm_alloc() {
  // alloc a new pgdir, map memory under PHY_MEM identityly
...
void vm_teardown(PD *pgdir) {
  //  free all pages mapping above PHY_MEM in pgdir, then free itself
...
PTE *vm_walkpte(PD *pgdir, size_t va, int prot) {
  //  return the pointer of PTE which match va
...
void vm_map(PD *pgdir, size_t va, size_t len, int prot) {
  // map [PAGE_DOWN(va), PAGE_UP(va+len)) at pgdir, with prot
...
void vm_unmap(PD *pgdir, size_t va, size_t len) {
  // unmap and free [va, va+len) at pgdir
  // you can just do nothing :)
...
void vm_copycurr(PD *pgdir) {
  //copy memory mapped in curr pd to pgdir
  //
